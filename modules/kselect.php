<?php
/**
 ** A Kimera enhanced module for [select] and [select*]
 **/

/* Tag generator */

add_action( 'wpcf7_admin_init', 'wpcf7_add_tag_generator_kselect', 26 );
function wpcf7_add_tag_generator_kselect() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'kselect', __( 'drop-down menu (from CRM)', 'cf7-kimera-crm' ),
		'wpcf7_tag_generator_kselect' );
}

function wpcf7_tag_generator_kselect( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );

	$description = __( "Generate a form-tag for a drop-down menu retrieving the values from the CRM.", 'cf7-kimera-crm' );
        ...
        <table class="form-table">
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="cf7k_resource" class="option idvalue" id="<?php echo esc_attr( $args['content'] . '-cf7k_resource' ); ?>" /></td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="<?php echo esc_attr( $args['content'] . '-cf7k_filter' ); ?>"><?php echo esc_html( __( 'Filter', 'cf7-kimera-crm' ) ); ?></label></th>
                    <td>
                        <input type="text" name="cf7k_filter" class="option idvalue" id="<?php echo esc_attr( $args['content'] . '-cf7k_filter' ); ?>" /></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <fieldset>
                            ...
                            <label>
                                <input type="checkbox" name="first_as_label" class="option" />
                                <?php echo esc_html( __( 'Use the first value as a label', 'cf7-kimera-crm' ) ); ?></label><br />
                            <label>
                                <input type="checkbox" name="multiple" class="option" />
                                <?php echo esc_html( __( 'Allow multiple selections', 'contact-form-7' ) ); ?></label><br />
                            <label>
                                <input type="checkbox" name="include_blank" class="option" />
                                <?php echo esc_html( __( 'Insert a blank item as the first option', 'contact-form-7' ) ); ?></label>
                        </fieldset>
                    </td>
                </tr>
